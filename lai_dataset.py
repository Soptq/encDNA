...
def get_chm_info(genetic_map, variants_pos, chm):
    ...
    genomic_intervals = scipy.interpolate.interp1d(x=genetic_chm["pos"].to_numpy(), y=genetic_chm["pos_cm"].to_numpy(),
...
class LAIDataset:
    ...
    def build_dataset(self, sample_map, sample_weights=None):
        """
    ...
    def create_splits(self, splits, outdir=None):
        ...
        if outdir is not None:
            for split in splits:
                ...
                self.return_split(split)[["sample", "population"]].to_csv(split_file, sep="\t", header=False,
                                                                          index=False)
